...
const historyFilePath = path.join(process.cwd(), 'public/history.json');
...
export async function GET() {
    try {
        ...
        return NextResponse.json({error: `Failed to load history: ${error.message}`}, {status: 500});
    }
}
...
export async function DELETE() {
    try {
        ...
        return NextResponse.json({error: `Failed to clear history: ${error.message}`}, {status: 500});
    }
}
